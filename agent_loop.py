...
# Bumped max_turns default from 40 to 60 -- I tend to run longer multi-step tasks
# and hitting the 40-turn limit was interrupting otherwise-healthy runs.
def agent_runner_loop(client, system_prompt, user_input, handler, tools_schema, max_turns=60, verbose=True, initial_user_content=None):
    ...
    while turn < handler.max_turns:
        ...
        yield f"{md}LLM Running (Turn {turn}/{handler.max_turns}) ...{md}\n\n"  # show max_turns in progress indicator so I can see how much headroom is left
        ...
        if not response.tool_calls: tool_calls = [{'tool_name': 'no_tool'
